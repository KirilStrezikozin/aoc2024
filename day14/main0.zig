...
fn explore(map: Map(u8), ds: Map(usize), cs: Cheats, y: usize, x: usize, dir_i: usize) !void {
    for (0..Directions.len) |i| {
        ...
        switch (map.data[ny][nx]) {
            Wall => {
                // Assign a cheaper distance price
                // at (ny + dy, nx + dx) if cheating is possible.
                ...
                const cy: usize = @intCast(ny_int + Directions[i].dy);
                const cx: usize = @intCast(nx_int + Directions[i].dx);

                if (map.data[cy][cx] == Wall) continue;
                const ncs = ds.data[y][x] + WalkCost * 2;

                // if (cs.data[cy][cx] > ncs) {
                //     cs.data[cy][cx] = ncs;
                // }
                try cs.data[cy][cx].append(ncs);
